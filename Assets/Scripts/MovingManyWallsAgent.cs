...
public class MovingManyWallsAgent : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        float moveAmount = (actions.ContinuousActions[1] + 1.0f) * 0.5f;
        transform.position += transform.forward * moveAmount * Time.deltaTime * moveSpeed;

        //penalty per step (1.0 / max step) * distance (current distance / total distance)
        AddReward(-((stepPenalty * 0.1f) + (stepPenalty * GetDistance()/totalDistance * 0.9f) * 2.0f) );
    }
